...
func _ready():
	...
	base_circle.size = Vector2(100, 100)
	base_circle.position = Vector2(-50, -50)
	base_circle.color = Color(0.3, 0.3, 0.3, 0.5)
	add_child(base_circle)
	
	# Create stick circle
	stick_circle = ColorRect.new()
	stick_circle.size = Vector2(50, 50)
	stick_circle.position = Vector2(-25, -25)
...
func _input(event):
	...
	if event is InputEventKey:
		...
		last_keyboard_time = Time.get_ticks_msec() / 1000.0
		update_visibility()
...
func update_visibility():
	...
	visible = DisplayServer.is_touchscreen_available() or not keyboard_mouse_detected
...
func update_stick_position(touch_pos):
	...
	stick_circle.position = center_position + offset - stick_circle.size / 2
...
func reset_stick():
	...
	stick_circle.position = center_position - stick_circle.size / 2
